...
extension Expression {
  init?(parse input: String) {
    ...
    func parseDouble(_ input: Stream) -> (Expression, Stream)? {
      ...
      func parseDouble(withPrefix prefix: String) -> (Expression, Stream) {
        let (succeeds, remainder) = split(rest, while: isNumeral)
    ...
    func parseVariable(_ input: Stream) -> (Expression, Stream)? {
      ...
      let (succeeds, remainder) = split(input, while: isAlpha)
    ...
    func split(_ str: Stream, while predicate: (Character) -> Bool) -> (Stream, Stream) {
      var taken = Stream()
      var remaining = str
      
      while let (char, rest) = decompose(remaining), predicate(char) {
        taken.append(char)
        remaining = rest
      }
    ...
    func drop(prefix drop: Stream, from str: Stream) -> Stream? {
  ...
  func evaluated(with env: [String:Double]) -> Double? {
    
    func combine(_ e1: Expression, _ e2: Expression,
                 with op: (Double, Double) -> Double) -> Double? {
      guard
        let n1 = e1.evaluated(with: env),
        let n2 = e2.evaluated(with: env)
        else { return nil }
      return op(n1, n2)
    }
    ...
    case let .paren(e): return e.evaluated(with: env)
    ...
    case let .sqrt(e): return e.evaluated(with: env).map(Foundation.sqrt)
    case let .sin(e): return e.evaluated(with: env).map(Foundation.sin)
    case let .cos(e): return e.evaluated(with: env).map(Foundation.cos)
    }
  }
